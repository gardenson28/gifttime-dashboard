  ...
  if (!trendResult || !surveyAnalysis) {
    return (
      <div className="space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">매칭 추천 리포트</h1>
          ...
          <EmptyNotice>
            <div className="space-y-2">
              ...
              <ul className="list-inside list-disc space-y-1">
                {!trendResult && (
                  <li>
                    <Link href="/research" className="text-rose-600 underline">
                      트렌드 리서치
                    </Link>
                    를 먼저 실행해주세요.
                  </li>
                ...
                {!surveyAnalysis && (
                  <li>
                    <Link href="/survey" className="text-rose-600 underline">
                      설문 분석
                    </Link>
                    을 먼저 실행해주세요.
                  </li>
    ...
    <div className="space-y-6">
      ...
      <Card>
        ...
        <div className="grid gap-3 sm:grid-cols-3 text-sm">
          <div>
            <p className="text-slate-500">시즌/이벤트</p>
            <p className="font-semibold text-slate-800">{trendResult.season}</p>
          </div>
          <div>
            <p className="text-slate-500">예산대</p>
            <p className="font-semibold text-slate-800">{trendResult.budget || "미지정"}</p>
          </div>
          <div>
            <p className="text-slate-500">설문 응답자 수</p>
            <p className="font-semibold text-slate-800">{surveyAnalysis.total}명</p>
          </div>
        </div>
      </Card>
      ...
      <Card>
        ...
        <p className="mt-2 text-sm text-slate-700">{surveyAnalysis.summary}</p>
      </Card>
        ...
        {!recommendations || recommendations.length === 0 ? (
          ...
          <div className="space-y-4">
            {recommendations.map((rec) => (
              <div key={rec.rank} className="rounded-lg border border-slate-200 p-4">
                <div className="flex flex-wrap items-center gap-2">
                  <span className="flex h-7 w-7 items-center justify-center rounded-full bg-rose-600 text-sm font-bold text-white">
                    {rec.rank}
                  </span>
                  <span className="text-lg font-bold text-slate-800">{rec.giftGroup} 선물군</span>
                  <Badge tone={rec.budgetFit ? "green" : "amber"}>
                ...
                <p className="mt-1 text-xs text-slate-500">설문 근거: {rec.refSurvey}</p>
              </div>
            ))}
          </div>
        )}
